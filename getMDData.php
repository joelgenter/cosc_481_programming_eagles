...
for ($x_position = 0, $new_line = ''; fseek($file, $x_position, SEEK_END) !== -1; $x_position--) {
		...
		if ($char === "\n") {
			$bottom_line = $new_line;
				...
				if(preg_match('/\s+/',$bottom_line)){
					$info = preg_split('/\s+/', $bottom_line);
					array_unshift($data,floatval($info[2]));
					array_unshift($points,floatval($info[1]));
					if(floatval($info[1])==0){
						break;
					}
				}
