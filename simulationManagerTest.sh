...
while true; do
  ...
  if [ ! -z "$query_result" ]; then    #if result not empty
    ...
    #i have to create these files because they aren't be generated when the gromacs commands above are commented out
    touch bar.xvg
    touch md_potential.xvg
    touch md_temperature.xvg
    touch md_pressure.xvg
    touch md_density.xvg
